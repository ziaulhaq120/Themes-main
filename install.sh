...
check_token() {
  ...
  if [ "$USER_TOKEN" = "zia" ]; then
    ...
    echo -e "${YELLOW}TELEGRAM : @zia${NC}"
    echo -e "${YELLOW}WHATSAPP : +6281376872631${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 25K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© zia${NC}"
    exit 1
  fi
  clear
}
...
while true; do
  ...
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : zia      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : zia  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by zia  ${NC}"
